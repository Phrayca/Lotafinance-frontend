...
import { obtenirMesMessagesAssistance, envoyerMessageAssistance, telechargerFichierChat, MessageAssistance } from "@/lib/api";
...
export default function PageAssistance() {
  ...
  useEffect(() => {
    ...
    obtenirMesMessagesAssistance(token)
      .then(setMessages)
      .catch((err) => setErreur(err instanceof Error ? err.message : "Une erreur est survenue"))
      .finally(() => setChargement(false));
    // Marque la conversation comme vue (pour la cloche de notifications)
  ...
  async function gererEnvoi(e: React.FormEvent) {
    ...
    setErreur("");
    try {
      const nouveauMessage = await envoyerMessageAssistance(token, saisie.trim(), fichier || undefined);
      ...
      <div className="max-w-xl mx-auto">
        ...
        <div className="bg-[#12151C] border border-[#232733] rounded-lg flex flex-col h-[70vh]">
          <div className="px-5 py-4 border-b border-[#232733]">
            <h1 className="font-['Source_Serif_4',serif] text-lg text-[#E8E6DE]">Assistance</h1>
            <p className="text-xs text-[#7C8494] mt-0.5">Une question ? Écrivez-nous, nous vous répondrons rapidement.</p>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
            {chargement ? (
              ...
              <p className="text-sm text-[#5A6070] text-center py-8">
                    ...
                    <div className={`flex ${m.auteur === "client" ? "justify-end" : "justify-start"}`}>
                      <div
                        className={`max-w-[75%] rounded-lg px-3 py-2 text-sm ${
                          m.auteur === "client" ? "bg-[#C9A227] text-[#0B0E14]" : "bg-[#1B2030] text-[#E8E6DE] border border-[#232733]"
                        }`}
                      >
                        ...
                        {m.fichier_nom && (
                          <button
                            ...
                            className={`flex items-center gap-1.5 text-xs mt-1 underline ${m.auteur === "client" ? "text-[#5A4A0A]" : "text-[#C9A227]"}`}
                          >
                            📎 {m.fichier_nom}
                          </button>
                        )}
                        <p className={`text-[10px] mt-1 ${m.auteur === "client" ? "text-[#5A4A0A]" : "text-[#7C8494]"}`}>
