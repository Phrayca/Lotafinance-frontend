...
import { recupererMonProfilUtilisateur } from "@/lib/api";
...
const LIENS_NAV = [
  ...
  { href: "/support/notifications", label: "Notifications", icone: "bell" as const, actif: true },
...
export default function PageNotifications() {
  ...
  const [nouveauxTickets, setNouveauxTickets] = useState(true);
  const [reponsesClient, setReponsesClient] = useState(true);
  ...
  useEffect(() => {
    ...
    setNouveauxTickets(localStorage.getItem("notif_nouveaux_tickets") !== "0");
    setReponsesClient(localStorage.getItem("notif_reponses_client") !== "0");
  }, [router]);
  ...
  function basculerNouveaux() {
    const v = !nouveauxTickets;
    setNouveauxTickets(v);
    localStorage.setItem("notif_nouveaux_tickets", v ? "1" : "0");
  }
  function basculerReponses() {
    const v = !reponsesClient;
    setReponsesClient(v);
    localStorage.setItem("notif_reponses_client", v ? "1" : "0");
  }
