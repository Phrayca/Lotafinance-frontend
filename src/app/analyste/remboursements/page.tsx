...
function estEnRetard(e: EcheanceGlobale) {
...
const LIENS_NAV = [
  ...
  { href: "/analyste/remboursements", label: "Remboursements", icone: "calendarCheck" as const, actif: true },
...
const FILTRES = [
  ...
  { valeur: "declarees", label: "Déclarées à vérifier" },
  { valeur: "retard", label: "En retard" },
  { valeur: "a_venir", label: "À venir" },
  { valeur: "payees", label: "Confirmées" },
];
...
export default function PageRemboursements() {
  ...
  const [echeances, setEcheances] = useState<EcheanceGlobale[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const liste = await obtenirTousLesRemboursements(token);
        setEcheances(liste);
      } catch (err) {
  ...
  async function gererConfirmation(e: EcheanceGlobale) {
    const token = localStorage.getItem("token");
    if (!token) return;
    setConfirmationEnCours(e.id);
    try {
      const misAJour = await confirmerPaiementEcheance(token, e.loan_id, e.id);
      setEcheances((precedent) =>
        precedent.map((x) => (x.id === e.id ? { ...x, payee: misAJour.payee, payee_le: misAJour.payee_le } : x))
      );
    } catch (err) {
      setErreur(err instanceof Error ? err.message : "Erreur lors de la confirmation");
    } finally {
  ...
  const echeancesFiltrees = echeances.filter((e) => {
    if (filtre === "declarees") return !e.payee && !!e.declaration_reference;
    if (filtre === "retard") return estEnRetard(e);
    if (filtre === "a_venir") return !e.payee && !estEnRetard(e);
    if (filtre === "payees") return e.payee;
    return true;
  });

  const totalRetard = echeances.filter(estEnRetard).length;
  const totalAVenir = echeances.filter((e) => !e.payee && !estEnRetard(e)).length;
  const totalPayees = echeances.filter((e) => e.payee).length;
    ...
    <main style={FOND_TEXTURE_STYLE} className="min-h-screen flex">
      ...
      <div className="flex-1 px-8 py-6 overflow-x-auto">
        <div className="max-w-3xl">
          ...
          <div className="mb-4">
            <h1 className="font-['Source_Serif_4',serif] text-2xl text-[#E8E6DE]">Remboursements</h1>
            <p className="text-[#7C8494] text-sm mt-1">Toutes les échéances, tous clients confondus</p>
          </div>

          <div className="grid grid-cols-3 gap-4 mb-4">
            <div className="bg-[#12151C] border border-[#232733] rounded-lg px-4 py-3">
              <p className="text-xs text-[#7C8494]">En retard</p>
              <p className="text-lg font-mono font-medium text-[#F0A0A0]">{totalRetard}</p>
            </div>
            <div className="bg-[#12151C] border border-[#232733] rounded-lg px-4 py-3">
              <p className="text-xs text-[#7C8494]">À venir</p>
              <p className="text-lg font-mono font-medium text-[#C9A227]">{totalAVenir}</p>
            </div>
            <div className="bg-[#12151C] border border-[#232733] rounded-lg px-4 py-3">
              <p className="text-xs text-[#7C8494]">Confirmées</p>
              <p className="text-lg font-mono font-medium text-[#3DDC97]">{totalPayees}</p>
            </div>
          </div>
          ...
          <div className="bg-[#12151C] border border-[#232733] rounded-lg p-6">
            ...
            {chargement ? (
              ...
              <p className="text-sm text-[#5A6070] py-8 text-center">Aucune échéance pour ce filtre.</p>
            ) : (
              <div className="space-y-2">
                {echeancesFiltrees.map((e) => {
                  const retard = estEnRetard(e);
                  const declaree = !e.payee && !!e.declaration_reference;
                  return (
                    <div
                      key={e.id}
                      className={`flex items-center justify-between border rounded-md p-3 ${
                        declaree ? "border-[#3A3013] bg-[#1B1706]" : retard ? "border-[#4A2222] bg-[#1A0F0F]" : "border-[#232733]"
                      }`}
                    >
                      <button onClick={() => router.push(`/analyste/${e.loan_id}`)} className="text-left min-w-0 hover:opacity-80 transition">
                        <p className="text-sm font-medium text-[#E8E6DE] truncate">{e.client_first_name} {e.client_last_name}</p>
                        <p className="text-xs text-[#7C8494] mt-0.5 font-mono">
                          Mensualité {e.numero} — {formaterMontant(e.montant)}
                          {e.frais_retard > 0 && <span className="text-[#F0A0A0]"> + {formaterMontant(e.frais_retard)} de retard</span>}
                        </p>
                        {declaree ? (
                          <p className="text-xs mt-0.5 text-[#C9A227]">
                            ⏳ Déclaré via {e.declaration_canal === "wave" ? "Wave" : "Orange Money"} — réf. {e.declaration_reference}
                          </p>
                        ) : (
                          <p className={`text-xs mt-0.5 ${retard ? "text-[#F0A0A0]" : "text-[#5A6070]"}`}>
                            {e.payee ? `Confirmée le ${e.payee_le ? formaterDate(e.payee_le) : ""}` : retard ? `En retard — échéance du ${formaterDate(e.date_echeance)}` : `Échéance : ${formaterDate(e.date_echeance)}`}
                          </p>
                        )}
                      </button>
                      {e.payee ? (
                        <span className="text-[#3DDC97] font-medium text-sm shrink-0">✓ Confirmée</span>
                      ) : (
                        <button
                          onClick={() => gererConfirmation(e)}
                          disabled={confirmationEnCours === e.id}
                          className="shrink-0 bg-[#1E4A3D] text-[#3DDC97] border border-[#2A6B57] text-xs font-medium px-3 py-1.5 rounded-md hover:bg-[#245A49] transition disabled:opacity-50"
                        >
                          {confirmationEnCours === e.id ? "..." : "Confirmer"}
                        </button>
                      )}
                    </div>
                  );
