...
const LIENS_NAV = [
  ...
  { href: "/analyste/utilisateurs", label: "Gestion des utilisateurs", icone: "users" as const, actif: true },
...
export default function PageGestionUtilisateurs() {
  ...
  async function gererChangementRole(u: UtilisateurApercu, nouveauRole: string) {
    if (nouveauRole === u.role) return;
    const token = localStorage.getItem("token");
    if (!token) return;
    setModificationEnCours(u.id);
    setErreur("");
    try {
      const misAJour = await modifierRoleUtilisateur(token, u.id, nouveauRole);
      setUtilisateurs((precedent) => precedent.map((x) => (x.id === u.id ? misAJour : x)));
    } catch (err) {
      setErreur(err instanceof Error ? err.message : "Erreur lors du changement de rôle");
    } finally {
          ...
          {chargement ? (
            ...
            <>
              ...
              <div className="bg-[#12151C] border border-[#232733] rounded-lg p-6">
                <table className="w-full text-sm border-collapse min-w-[500px]">
                  <thead>
                    <tr className="text-left text-[10px] uppercase tracking-wide text-[#7C8494] border-b border-[#232733]">
                      ...
                      <th className="pb-2 font-medium">Changer le rôle</th>
                    </tr>
                  </thead>
                  <tbody>
                    {utilisateurs.map((u) => {
                      const couleur = couleurRole(u.role);
                      return (
                        <tr key={u.id} className="border-b border-[#1B1F29] last:border-0">
                          <td className="py-3 pr-3 text-[#E8E6DE] font-mono">{u.email}</td>
                          ...
                          <td className="py-3 pr-3 text-[#7C8494] text-xs whitespace-nowrap">{formaterDate(u.created_at)}</td>
                          <td className="py-3">
                            <select
                              value={u.role}
                              disabled={modificationEnCours === u.id || u.id === utilisateur?.id}
                              onChange={(e) => gererChangementRole(u, e.target.value)}
                              className="bg-[#0B0E14] border border-[#232733] rounded-md px-2 py-1.5 text-xs text-[#E8E6DE] focus:outline-none focus:border-[#C9A227] transition disabled:opacity-50"
                            >
                              {ROLES.map((r) => (
                                <option key={r} value={r}>{libelleRole(r)}</option>
                              ))}
                            </select>
                          </td>
                        </tr>
                ...
                <p className="text-[11px] text-[#5A6070] mt-4">Tu ne peux pas modifier ton propre rôle depuis cette page (sécurité).</p>
              </div>
            </>
          )}
        </div>
