...
const LIENS_NAV = [
  ...
  { href: "/analyste/analyse-scoring", label: "Analyse & Scoring", icone: "target" as const, actif: true },
...
export default function PageAnalyseScoring() {
  ...
  const [donnees, setDonnees] = useState<ScoringApercu[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const liste = await obtenirScoringPortefeuille(token);
        setDonnees(liste);
      } catch (err) {
    ...
    <main style={FOND_TEXTURE_STYLE} className="min-h-screen flex">
      ...
      <div className="flex-1 px-8 py-6 overflow-x-auto">
        <div className="max-w-4xl">
          ...
          <div className="mb-6">
            <h1 className="font-['Source_Serif_4',serif] text-2xl text-[#E8E6DE]">Analyse & Scoring</h1>
            <p className="text-[#7C8494] text-sm mt-1">Détail du score de solvabilité de tous les dossiers</p>
          </div>
              ...
              <table className="w-full text-sm border-collapse min-w-[700px]">
                <thead>
                  <tr className="text-left text-[10px] uppercase tracking-wide text-[#7C8494] border-b border-[#232733]">
                    ...
                    <th className="pb-2 font-medium">Ancienneté</th>
                    <th className="pb-2 font-medium">Capacité</th>
                    <th className="pb-2 font-medium">Résidence</th>
                    <th className="pb-2 font-medium">Historique</th>
                  </tr>
                </thead>
                <tbody>
                  {donnees.map((d) => {
                    const cs = couleurStatut(d.status);
                    const cr = couleurRisque(d.risk_level);
                    return (
                      <tr key={d.loan_id} onClick={() => router.push(`/analyste/${d.loan_id}`)} className="border-b border-[#1B1F29] last:border-0 cursor-pointer hover:bg-[#171B24] transition">
                        <td className="py-3 pr-3 text-[#E8E6DE] font-medium whitespace-nowrap">{d.client_first_name} {d.client_last_name}</td>
                        <td className="py-3 pr-3"><span className="text-xs font-medium px-2.5 py-1 rounded-full whitespace-nowrap" style={{ backgroundColor: cs.bg, color: cs.text }}>{LIBELLES_STATUT[d.status] || d.status}</span></td>
                        <td className="py-3 pr-3 font-mono text-[#E8E6DE]">{d.credit_score ?? "—"}</td>
                        <td className="py-3 pr-3"><span className="text-xs font-medium px-2.5 py-1 rounded-full whitespace-nowrap" style={{ backgroundColor: cr.bg, color: cr.text }}>{d.risk_level ?? "—"}</span></td>
                        <td className="py-3 pr-3 font-mono text-[#B8BAC4]">{d.profession_score ?? "—"}</td>
                        <td className="py-3 pr-3 font-mono text-[#B8BAC4]">{d.anciennete_score ?? "—"}</td>
                        <td className="py-3 pr-3 font-mono text-[#B8BAC4]">{d.capacity_score ?? "—"}</td>
                        <td className="py-3 pr-3 font-mono text-[#B8BAC4]">{d.residence_score ?? "—"}</td>
                        <td className="py-3 font-mono text-[#B8BAC4]">{d.history_score ?? "—"}</td>
                      </tr>
                    );
                  })}
                </tbody>
