...
const LIENS_NAV = [
  ...
  { href: "/analyste/parametres", label: "Paramètres", icone: "gear" as const, actif: true },
...
export default function PageParametres() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const liste = await obtenirTaux(token);
        setTaux(liste);
        setValeurs(Object.fromEntries(liste.map((t) => [t.id, String(t.rate_percent)])));
      } catch (err) {
  ...
  async function gererEnregistrement(t: Taux) {
    const token = localStorage.getItem("token");
    if (!token) return;
    const nouvelleValeur = Number(valeurs[t.id]);
    if (!nouvelleValeur || nouvelleValeur <= 0) {
      setErreur("Merci d'indiquer un taux valide.");
      return;
    }
    setErreur("");
    setSucces("");
    setEnregistrementEnCours(t.id);
    try {
      const misAJour = await modifierTaux(token, t.id, nouvelleValeur);
      setTaux((precedent) => precedent.map((x) => (x.id === t.id ? misAJour : x)));
      setSucces(`Taux à ${misAJour.duration_weeks} semaine(s) mis à jour.`);
    } catch (err) {
    ...
    <main style={FOND_TEXTURE_STYLE} className="min-h-screen flex">
      ...
      <div className="flex-1 px-8 py-6">
        <div className="max-w-2xl">
          ...
          <div className="mb-6">
            <h1 className="font-['Source_Serif_4',serif] text-2xl text-[#E8E6DE]">Paramètres</h1>
            <p className="text-[#7C8494] text-sm mt-1">Taux appliqués selon la durée du prêt</p>
          </div>

          {!estAdmin && (
            <div className="flex items-center gap-2 bg-[#1B1706] border border-[#3A3013] rounded-md px-4 py-3 mb-4 text-sm text-[#C9A227]">
              {Ic("lock", "w-4 h-4 shrink-0")} Lecture seule — seul un compte admin peut modifier ces taux.
            </div>
          )}
          {erreur && <p className="text-sm text-[#F0A0A0] bg-[#2A1414] border border-[#4A2222] rounded-md px-3 py-2 mb-4">{erreur}</p>}
          {succes && <p className="text-sm text-[#3DDC97] bg-[#0F2420] border border-[#1E4A3D] rounded-md px-3 py-2 mb-4">{succes}</p>}
          ...
          <div className="bg-[#12151C] border border-[#232733] rounded-lg p-6">
            {chargement ? (
              ...
              <div className="space-y-3">
                {taux.map((t) => (
                  <div key={t.id} className="flex items-center justify-between gap-4 border border-[#232733] rounded-md p-4">
                    <div>
                      <p className="text-sm font-medium text-[#E8E6DE]">{t.duration_weeks} semaine{t.duration_weeks > 1 ? "s" : ""}</p>
                      <p className="text-xs text-[#7C8494]">Taux standard appliqué</p>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="relative">
                        <input
                          ...
                          disabled={!estAdmin}
                          value={valeurs[t.id] ?? ""}
                          onChange={(e) => setValeurs((v) => ({ ...v, [t.id]: e.target.value }))}
                          className="w-24 bg-[#0B0E14] border border-[#232733] rounded-md pl-3 pr-7 py-2 text-sm text-[#E8E6DE] text-right focus:outline-none focus:border-[#C9A227] transition disabled:opacity-50"
                        />
                        <span className="absolute right-2.5 top-1/2 -translate-y-1/2 text-xs text-[#7C8494]">%</span>
                      </div>
                      {estAdmin && (
                        <button
                          onClick={() => gererEnregistrement(t)}
                          disabled={enregistrementEnCours === t.id}
                          className="text-xs font-medium bg-[#C9A227] text-[#0B0E14] px-3 py-2 rounded-md hover:bg-[#DDB63A] transition disabled:opacity-50"
                        >
                          {enregistrementEnCours === t.id ? "..." : "Enregistrer"}
                        </button>
                      )}
                    </div>
                  </div>
            ...
            <p className="text-[11px] text-[#5A6070] mt-4">
              Rappel : la facilité de paiement majore le taux de 10 points supplémentaires (fixé dans le code, pas modifiable ici).
            </p>
          </div>
        </div>
      </div>
