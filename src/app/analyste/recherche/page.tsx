...
function PageRechercheContenu() {
  ...
  useEffect(() => {
    const token = localStorage.getItem("token");
    if (!token || !requeteInitiale.trim()) {
      setChargement(false);
      return;
    }
    setChargement(true);
    rechercherDossiers(token, requeteInitiale)
      .then(setResultats)
    ...
    <main style={FOND_TEXTURE_STYLE} className="min-h-screen flex">
      ...
      <div className="flex-1 px-8 py-6">
        <div className="max-w-3xl">
          {/* En-tête : recherche + messages + profil */}
          ...
          <div className="bg-[#12151C] border border-[#232733] rounded-lg p-6">
            ...
            <p className="text-[#7C8494] text-sm mb-6">
              {requeteInitiale ? `Pour « ${requeteInitiale} »` : "Tape une recherche ci-dessus"}
              {!chargement && requeteInitiale ? ` — ${resultats.length} résultat${resultats.length > 1 ? "s" : ""}` : ""}
            </p>
            ...
            ) : resultats.length === 0 && requeteInitiale ? (
              <p className="text-sm text-[#5A6070] py-8 text-center">Aucun dossier ne correspond à cette recherche.</p>
            ) : (
              <div className="space-y-3">
                {resultats.map((r) => {
                  const couleur = couleurStatut(r.status);
                  return (
                    <button
                      key={r.id}
                      onClick={() => router.push(`/analyste/${r.id}`)}
                    ...
                    >
                      <div>
                        <p className="text-sm font-medium text-[#E8E6DE]">{r.client_first_name} {r.client_last_name}</p>
                        <p className="text-xs text-[#7C8494] mt-0.5 font-mono">{r.client_phone} · {formaterMontant(r.amount_requested)}</p>
                      </div>
