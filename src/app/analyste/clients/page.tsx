...
export default function PageClientsAnalyste() {
  ...
  const clientsFiltres = filtreTexte.trim()
    ? clients.filter((c) => {
      ...
      <div className="flex-1 px-8 py-6">
        <div className="max-w-3xl">
          ...
          <div className="bg-[#12151C] border border-[#232733] rounded-lg p-6">
            <h1 className="font-['Source_Serif_4',serif] text-xl text-[#E8E6DE] mb-1">Clients</h1>
            <p className="text-[#7C8494] text-sm mb-4">
              {clients.length} client{clients.length > 1 ? "s" : ""} Lotafinance
            </p>
            ...
            <input
              ...
              className="w-full bg-[#0B0E14] border border-[#232733] rounded-md px-3 py-2 text-sm text-[#E8E6DE] placeholder-[#5A6070] focus:outline-none focus:border-[#C9A227] transition mb-5"
            />
            ...
            {chargement ? (
              ...
              <p className="text-sm text-[#5A6070] py-8 text-center">Aucun client ne correspond.</p>
            ) : (
              <div className="space-y-2">
                {clientsFiltres.map((c) => {
                  const couleur = couleurRisque(c.dernier_risque);
                  return (
                    <button
                      key={c.id}
                      onClick={() => router.push(`/analyste/clients/${c.id}`)}
                    ...
                    >
                      <div className="min-w-0">
                        <p className="text-sm font-medium text-[#E8E6DE]">{c.first_name} {c.last_name}</p>
                        <p className="text-xs text-[#7C8494] mt-0.5 font-mono truncate">{c.phone} · {c.email}</p>
                        <p className="text-[11px] text-[#5A6070] mt-0.5">
                          Client depuis le {formaterDate(c.created_at)} · {c.nombre_prets} prêt{c.nombre_prets > 1 ? "s" : ""}
                        </p>
                      </div>
                      <div className="flex flex-col items-end gap-1.5 shrink-0">
                        {c.identity_verified ? (
                          <span className="inline-flex items-center gap-1 text-[10px] font-medium text-[#3DDC97] bg-[#0F2420] border border-[#1E4A3D] rounded-full px-2 py-0.5">
                            {Ic("idCheck", "w-3 h-3")} Vérifiée
                          </span>
                        ) : c.identity_rejected ? (
                          <span className="inline-flex items-center gap-1 text-[10px] font-medium text-[#F0A0A0] bg-[#2A1414] border border-[#4A2222] rounded-full px-2 py-0.5">
                            {Ic("idX", "w-3 h-3")} Rejetée
                          </span>
                        ) : (
                          <span className="text-[10px] font-medium text-[#7C8494] bg-[#1B1F29] rounded-full px-2 py-0.5">Identité en attente</span>
                        )}
                        {c.dernier_risque && (
                          <span className="text-[10px] font-medium px-2 py-0.5 rounded-full" style={{ backgroundColor: couleur.bg, color: couleur.text }}>
                            {c.dernier_risque}{c.dernier_score != null ? ` · ${c.dernier_score}` : ""}
                          </span>
                        )}
                      </div>
                    </button>
